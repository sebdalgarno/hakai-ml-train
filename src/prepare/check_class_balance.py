"""Check class distribution across chip dataset."""
...
def generate_stats_pdf(
    ...
    """Generate chip counts and class balance stats, print to console and optionally to PDF.

    Can write to a standalone PDF (via output_path) or append to an existing PdfPages
    object (via pdf). Returns the collected per-split stats for reuse.
    """
    if class_names is None:
        class_names = {0: "bg", 1: "class_1"}
    ...
    # Collect stats once — get_class_balance_stats already counts chips
    ...
    # Build PDF page
    def _write_stats_page(target_pdf: PdfPages) -> None:
        ...
        plt.tight_layout()
        target_pdf.savefig(fig)
        plt.close(fig)

    if pdf is not None:
        _write_stats_page(pdf)
    elif output_path is not None:
        with PdfPages(output_path) as standalone_pdf:
            _write_stats_page(standalone_pdf)
...
def generate_report(
    ...
    with PdfPages(output_path) as pdf:
        # Page 1: Chip counts + class balance (delegate to generate_stats_pdf)
        generate_stats_pdf(
            chip_dir,
