...
def generate_stats_pdf(
    ...
    with PdfPages(output_path) as pdf:
        ...
        ax_balance = axes[1]
        ax_balance.axis("off")
        ax_balance.set_title("Class Balance", fontsize=14, fontweight="bold", loc="left")
    ...
    with PdfPages(output_path) as pdf:
        ...
        for split in splits:
            ...
            for page in range(n_pages):
                ...
                if page == 0:
                    fig.suptitle(f"{split.upper()} samples", fontsize=12, fontweight="bold")
                    legend_elements = [
                        plt.Line2D([0], [0], marker="s", color="w",
                                   markerfacecolor=cmap(i), markersize=10, label=name)
                        for i, name in class_names.items()
                    ]
                    legend_elements.append(
                        plt.Line2D([0], [0], marker="s", color="w",
                                   markerfacecolor="white", markeredgecolor="gray",
                                   markersize=10, label="ignore")
                    )
