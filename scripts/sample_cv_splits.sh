...
# Equal sampling per site (each site contributes same number of tiles)
set -e
...
# FUNCTIONS -----
sample_equal_per_site() {
    local src_dir="$1"
    local dst_dir="$2"
    local target="$3"
    local seed="$4"

    # Use Python for equal-per-site sampling
    python3 << EOF
import random
import shutil
...
def extract_site(filename):
    ...
    # Pattern: {site}_u{id}_{chip_idx} (e.g., koeye_u0388_42)
    match = re.match(r"(.+)_u\d+_\d+$", stem)
    if match:
        return match.group(1)
    # Pattern: {site}_{chip_idx} (e.g., bennett_bay_42)
...
# Equal samples per site: target / num_sites
per_site = max(1, target // num_sites)

dst.mkdir(parents=True, exist_ok=True)
sampled = 0

for site, files in sorted(by_site.items()):
    n_sample = min(per_site, len(files))
...
print(f"  Total: {sampled} tiles (target: {target}, {per_site}/site)")
EOF
}

# MAIN -----
echo "=============================================="
echo "Sampling CV Splits (Equal per Site)"
echo "=============================================="
...
for cv in "${CV_SPLITS[@]}"; do
    ...
    # Sample train (equal per site)
    echo ""
    echo "Sampling train (target: $TARGET_TRAIN, equal per site):"
    sample_equal_per_site "$SRC/train" "$DST/train" "$TARGET_TRAIN" "$SEED"

    # Sample val (equal per site)
    echo ""
    echo "Sampling val (target: $TARGET_VAL, equal per site):"
    sample_equal_per_site "$SRC/val" "$DST/val" "$TARGET_VAL" "$SEED"
